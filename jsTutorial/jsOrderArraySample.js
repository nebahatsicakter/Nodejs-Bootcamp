...
// * Brezilya dan kaç adet sipariş aldım

let brazilOrders = orders.filter((x) => x.shipAddress?.country == "Brazil");

console.log("Brazilya Sipariş Sayısı:", brazilOrders.length);
...
//   if (orders[i].shipAddress.country == "Brazil") {
//     for (let index = 0; index < orders[i].details.length; index++) {
//       var quantity = orders[i].details[index].quantity;
//       var price = orders[i].details[index].unitPrice;
//       total += quantity * price;
//     }
//   }
// }

brazilOrders.forEach((item) => {
  var orderPrice = 0;
  item.details.forEach((index) => {
    orderPrice = orderPrice + index.unitPrice * index.quantity;
  });
  total += orderPrice;
});

console.log("Brezilya Toplam Sipariş Tutarı: ", total.toFixed(2));

// * Brezilyadan aldığım en yüksek sipariş
...
brazilOrders.forEach(function (item) {
  ...
  if (maxOrderPrice < orderPrice) {
    maxOrderPrice = orderPrice;
  }
});
console.log("Maksimum sipariş: ", maxOrderPrice);
